...
from Clases.Invasor import Invasor as Enemigo
...
def detenerTodo2():
    for jugador in Enemigo.listaDisparo:
    ...
    while True:
        ...
        if enJuego == False:
            ...
            if event.type == KEYDOWN:
                # Por alguna razón no captura la tecla R
                if event.key == K_r:
                    pygame.quit()
                    SpaceInvader()
...
Si te gustó el juego, te lo regalo, ya que es OpenSource.
Lo he creado sólo para ganar experiencia en los videojuegos en Python.
Si estas en un proyecto en Python puedo ayudarte, sólo escríbeme un DM en X:
https://www.twitter.com/juniorsverr

Ahí estoy para cualquier duda.
Con gusto Server1567 :/
